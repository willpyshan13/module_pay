...
public class WsPayOrderServer {
    ...
    public void onOpen(Session session, @PathParam("payOrderId") String payOrderId, @PathParam("cid") String cid) {
        ...
        try {
            //设置当前属性
            this.cid = cid;
            this.payOrderId = payOrderId.length() > 22 ? payOrderId.substring(0, 22) : payOrderId;
    ...
    @OnClose
    public void onClose() {
        Set<WsPayOrderServer> wsSet = wsOrderIdMap.get(this.payOrderId);
        wsSet.remove(this);
        if (wsSet.isEmpty()) {
        ...
        try {
            updatePayOrder(this.payOrderId);
        } catch (Exception e) {
            logger.error("更新支付订单状态异常 payOrderId=" + this.payOrderId, e);
        }
    ...
    public synchronized void updatePayOrder(String payOrderId) {
